...
export class ActivityStateService {
  ...
  startActivity(classroomId: string, assignments: any[]): Activity | null {
    ...
    if (activity && activity.status === 'waiting') {
      ...
      console.log(
        `[ActivityStateService] Activity status changed to 'active' for room ${classroomId}.`,
      );
      return activity;
    }
  ...
  updateUserSubmission(
    ...
    if (activity && activity.status === 'active') {
      ...
      console.log(
        `[ActivityStateService] Submission updated for user ${userId} in room ${classroomId}.`,
      );
      return activity;
    }
