...
export function OverviewPage() {
  ...
  /** 막대의 정체성은 배열 위치가 아니라 "시각"이다. key 로 쓰려고 값에 담아 둔다. */
  const bars = data.hourlyOnline.map((online, hour) => ({ hour, online }));
      ...
      <div className='grid gap-5 lg:grid-cols-2'>
        <div className='panel p-5'>
          ...
          <div className='flex h-24 items-end gap-[3px]'>
            {bars.map(({ hour, online }) => (
              <div
                key={hour}
                className='flex-1 rounded-t'
                style={{
                  height: `${Math.max(3, (online / peak) * 96)}px`,
                  background: hour === peakHour ? 'var(--color-algo)' : 'var(--color-data)',
                }}
                title={`${String(hour).padStart(2, '0')}시 · ${online}명`}
              />
            ))}
          </div>
        ...
        <div className='panel p-5'>
          ...
          {data.services.map((s) => (
            <div
              key={s.name}
              className='flex items-center gap-3 border-b border-dashed border-line py-2.5 text-sm last:border-0'
            >
              <span
                className='rounded-full border px-2.5 py-0.5 text-xs font-semibold'
                style={{
                  borderColor: `var(${STATUS[s.status].cssVar})`,
                  color: `var(${STATUS[s.status].cssVar})`,
                }}
              >
