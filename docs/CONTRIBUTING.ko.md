...
5. **커밋 메시지는 `타입(범위): 제목` + 요약 + 불릿으로 쓴다:**

   ```
   <타입>(<범위>): <제목: 명령형, 줄 전체 72자 이하>
   ...
   타입은 `feat`, `fix`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`, `chore`,
   `release` 중 하나다. 파괴적 변경은 콜론 앞에 `!` 를 붙인다. 범위는 선택이며 영역을 적는다 —
   `api`, `web`, `shared`, `db`, `auth`, `docker`, `deps`. 제목 줄은 `commit-msg` hook 이 검사한다.
...
전체 매핑 규칙은 팀 내부 작업 공간에 있다. 여기서 다루지 않은 경우는 이슈나 PR 에서 물어보면 된다.
